...
function buildTree(docs: DisplayDoc[]): TreeNode[] {
  ...
  const sortTree = (node: TreeNode) => {
    node.children.sort((a, b) => {
      if (a.type === b.type) {
        if (a.type === "file") {
          // Sort files by Year descending (newest first), then by name
          const yearA = parseInt(a.doc?.year || "0", 10);
          const yearB = parseInt(b.doc?.year || "0", 10);
